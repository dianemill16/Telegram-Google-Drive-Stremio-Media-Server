...
class Database:
    ...
    async def insert_media(
        ...
        if metadata_info['media_type'] == "movie":
            media = MovieSchema(
                ...
                telegram=[quality_obj] if source_type == "telegram" else [],
                gdrive=[quality_obj] if source_type == "gdrive" else []
            )
            return await self.update_movie(media, source_type)
        else:
            ...
            return await self.update_tv_show(tv_show, source_type)

    async def update_movie(self, movie_data: MovieSchema, source_type: str = "telegram") -> Optional[ObjectId]:
        try:
        ...
        source_list_name = "telegram" if source_type == "telegram" else "gdrive"
        quality_to_update = movie_dict[source_list_name][0]
        ...
        if not existing_movie:
            try:
                ...
                if any(keyword in str(e).lower() for keyword in ["storage", "quota"]):
                    return await self._handle_storage_error(self.update_movie, movie_data, source_type, total_storage_dbs=total_storage_dbs)
                return None

        # ---------------- UPDATE MOVIE ----------------
        movie_id = existing_movie["_id"]
        existing_qualities = existing_movie.get(source_list_name, [])

        if Telegram.REPLACE_MODE:
            to_delete = [q for q in existing_qualities if q.get("quality") == target_quality]
            
            # Delete old messages if telegram
        ...
        existing_movie[source_list_name] = existing_qualities
    ...
    async def update_tv_show(self, tv_show_data: TVShowSchema, source_type: str = "telegram") -> Optional[ObjectId]:
        try:
        ...
        if not existing_tv:
            try:
                ...
                LOGGER.error(f"Insertion failed in {current_db_key}: {e}")
                if any(keyword in str(e).lower() for keyword in ["storage", "quota"]):
                    return await self._handle_storage_error(self.update_tv_show, tv_show_data, source_type, total_storage_dbs=total_storage_dbs)
                return None
        ...
        for season in tv_show_dict["seasons"]:
            ...
            for episode in season["episodes"]:
                ...
                existing_episode.setdefault(source_list_name, [])

                for quality in episode[source_list_name]:
                    target_quality = quality.get("quality")

                    if Telegram.REPLACE_MODE:
                        to_delete = [
                            q for q in existing_episode[source_list_name]
                        ...
                        existing_episode[source_list_name] = [
                            q for q in existing_episode[source_list_name]
                            if q.get("quality") != target_quality
                        ]
                        existing_episode[source_list_name].append(quality)
            ...
            if season.get("season_number") == season_number:
                for ep in season["episodes"]:
                    if ep.get("episode_number") == episode_number:
                        # Logic to clear sub-qualities would go here
                        pass
        ...
        for season in tv["seasons"]:
            if season.get("season_number") == season_number:
                for episode in season["episodes"]:
                    if episode.get("episode_number") == episode_number:
                        # Check Telegram
                        if "telegram" in episode:
                            original_len = len(episode["telegram"])
                            episode["telegram"] = [q for q in episode["telegram"] if q.get("id") != id]
                            if len(episode["telegram"]) != original_len:
                                found = True
                        
                        # Check GDrive
                        if "gdrive" in episode:
                            original_len = len(episode["gdrive"])
                            episode["gdrive"] = [q for q in episode["gdrive"] if q.get("id") != id]
                            if len(episode["gdrive"]) != original_len:
                                found = True
                        break
