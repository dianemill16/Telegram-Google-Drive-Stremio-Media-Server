...
# ----------------- Main Metadata -----------------
async def metadata(filename: str, id_data: dict, is_gdrive: bool = False) -> dict | None:
    try:
    ...
    # GDrive file fallback for quality
    if not quality:
        if is_gdrive:
             quality = "Unknown"
        else:
    ...
    try:
        encoded_string = await encode_string(id_data)
    except Exception:
        encoded_string = None
...
async def fetch_tv_metadata(title, season, episode, encoded_string, year=None, quality=None, default_id=None) -> dict | None:
    ...
    must_use_tmdb = (use_tmdb or imdb_tv is None or imdb_tv == {})
...
async def fetch_movie_metadata(title, encoded_string, year=None, quality=None, default_id=None) -> dict | None:
    ...
    must_use_tmdb = (use_tmdb or imdb_details is None or imdb_details == {})
