...
async def process_file():
    while True:
        ...
        async with db_lock:
            # Modified to pass source_data and source_type
            source_data = {"size": size, "name": title}
            updated_id = await db.insert_media(metadata_info, source_data, source_type="telegram")
    ...
    if str(message.chat.id) in Telegram.AUTH_CHANNEL:
        try:
            if message.video or (message.document and message.document.mime_type.startswith("video/")):
                ...
                # Prepare ID data for Telegram
                id_data = {"chat_id": int(channel), "msg_id": msg_id}
                
                metadata_info = await metadata(clean_filename(title), id_data)
