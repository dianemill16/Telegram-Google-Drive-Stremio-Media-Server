...
async def stream_handler(request: Request, id: str, name: str):
    ...
    if "gdrive_id" in decoded_data:
        return await gdrive_streamer(request, decoded_data["gdrive_id"], name)
...
async def gdrive_streamer(request: Request, file_id: str, filename: str):
    range_header = request.headers.get("Range")
    
    meta = await gdrive_client.get_file_metadata(file_id)
    if not meta:
